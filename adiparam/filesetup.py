...
def choosefiles(msg="Choose the Raw Files to Use from the Following List.", 
    ...
    if 'HICA' in filelist[0]:   
        ...
        exp1time = [float(header['EXP1TIME']) for header in headerlist]
        exptime = [float(header['EXPTIME']) for header in headerlist]
        coadd = [int(header['COADD']) for header in headerlist]
